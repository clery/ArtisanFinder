...
if C_AddOns and C_AddOns.LoadAddOn and (not WhoFrame or not FriendsFrame) then
...
function lib.Who(defhandler, query, opts)
	...
	args.noRetry = opts.noRetry == true
...
local queryInterval = lib.MinInterval
...
function lib:AskWhoNext()
	...
	if self.WhoInProgress then
		...
		self.Args = nil
		if args.noRetry then
			dbg("Not retrying "..args.query)
			self:RaiseCallback(args, args.query, {}, false, args.info)
		else
...
function lib:ReturnWho()
	...
	local now = time()
	local complete = (self.Total == #self.Result) and (self.Total < (MAX_WHOS_FROM_SERVER or 50))
